 ...
 */
public class DynamicUpstreamTest {

    public NginxRpBuilder nginxReverseProxy;
    public AppServiceCluster dynamicUpstream = makeHttpsWebService("Dynamic_Cluster");

    @Before
    public void setup() {
        nginxReverseProxy = runNginxWithUpstreams(dynamicUpstream);
    }

    @Test
    public void should_properly_pass_url_fields() {
        try {
            make(getRequest("https://www.nginx-test.com/dynamic").to(nginxReverseProxy))
                    .andExpectClusterName("Dynamic_Cluster");
        } finally {
